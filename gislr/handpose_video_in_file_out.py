...
def run(model: str, num_hands: int,
        ...
        input_video_path: str, output_data_path: str, width: int, height: int) -> None:
...
#    cap.set(cv2.CAP_PROP_FRAME_HEIGHT, height)
    ...
    while cap.isOpened():
        ...
        if DETECTION_RESULT.hand_landmarks:
            ...
            # Append frame data to the output file
            with open(output_data_path, "a") as f:
                json.dump(frame_data, f, indent=4)
                f.write(",\n")  # Add a comma to separate frames
    ...
    with open(output_data_path, "a") as f:
        f.seek(f.tell() - 2)  # Remove the last comma
        f.write("\n]\n")

    print(f"Hand landmark data saved to {output_data_path}")
...
def main():
    ...
    parser.add_argument(
        '--jsonPath',
        help='Path to the output json data file.',
        required=False,
        type=str,
        default='json01.dat')
    parser.add_argument(
    ...
    run(args.model, args.numHands, args.minHandDetectionConfidence,
        ...
        args.videoPath, args.jsonPath, args.frameWidth, args.frameHeight)
